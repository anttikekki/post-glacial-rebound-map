...
#
#  "User_DTM@1" - ((2 / 3.14159 * ("Base Raster@1" * 0.075) * (
#     ATAN("Base Raster@2" / (5 * ("Base Raster@1" * 0.075) + 500)) - 
#     ATAN(("Base Raster@2" -1950 + [yearCE]) / (5 * ("Base Raster@1" * 0.075) + 500))
#  )) + 
#  ((("Base Raster@3" * 0.072) * ((2020 - [yearCE]) / 100)) - 
#  (0.5 * (-0.014 * (("Base Raster@3" * 0.072) * ((2020 - [yearCE]) / 100) ^ 2))))) 
#  - [sea-level ref]
#
# Parameters:
...
#   `2020`: recerence year of `User_DTM@1`. National land survey of Finland (NLS) elevation model is from 2023 but the difference is so small that this does not need to be alterered.
...
CALC_EXPR="${ORIGINAL_FORMULA//\[yearCE\]/$CALENDAR_YEAR}"
CALC_EXPR="${CALC_EXPR//\[sea-level ref\]/$SEA_LEVEL}"
...
CALC_EXPR="${CALC_EXPR//ATAN/atan}"  # convert ATAN → atan because Python’s numpy math engine uses lower case commands
CALC_EXPR="${CALC_EXPR//^/**}" # convert ^ → ** because Python’s numpy math engine syntax

echo "Final formula for gdal_calc: $CALC_EXPR"
...
export ALIGNED_BASE_FOLDER SEA_LEVEL_RASTER OUTPUT_FOLDER CALENDAR_YEAR CALC_EXPR
